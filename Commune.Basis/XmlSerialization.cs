  ...
  public class XmlSerialization
  {
    ...
    public static void SaveItems<T>(string holderName, ICollection<T> items, XmlWriter writer)
    {
      ...
      try
      {
        ...
        if (CollectionHlp.GetCount(items) > 0)
        {
    ...
    public static void Save(object obj, FileStreamInfo info)
    {
      using (XmlWriter writer = info.GetXmlWriter())
        Save(obj, writer);
    }
    ...
    public class DataStorage<TItem>
    {
      ...
      {
        this.Items = CollectionHlp.ToArray(items);
      }
      public DataStorage() { }
